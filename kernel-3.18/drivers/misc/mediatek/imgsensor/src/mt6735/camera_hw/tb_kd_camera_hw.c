	...
	if (On) {
		...
		if (pinSetIdx == 0 && currSensorName
			...
			mdelay(1);
			   ...
			   && (0 == strcmp(currSensorName, SENSOR_DRVNAME_GC2145_MIPI_YUV))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
						pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
			...
			/* because DVDD and DOVDD are tied together for 1.8V in module,
			* so don't need to power on DVDD
			*/
#if 0
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
			   ...
			   && (0 == strcmp(currSensorName, SENSOR_DRVNAME_GC0310_MIPI_YUV))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
						pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
